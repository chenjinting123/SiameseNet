...
#model_2:在原有模型的基础上，改变了卷积窗口的大小
convnet = Sequential()
convnet.add(Conv2D(64,(6,6),activation='relu',input_shape=input_shape,
                   kernel_initializer=W_init,kernel_regularizer=l2(2e-4),name='Conv_1'))
...
convnet.add(MaxPooling2D())
#convnet.add(Dropout(0.25))
convnet.add(Conv2D(128,(2,2),activation='relu',kernel_initializer=W_init,kernel_regularizer=l2(2e-4),bias_initializer=b_init))
convnet.add(MaxPooling2D())
#convnet.add(Dropout(0.25))
...
#plot_model(siamese_net, to_file='/home/cjt/keras_siamese/model.png')
#plot_model(convnet, to_file='/home/cjt/keras_siamese/model_2.png')

#取需要输出的中间层作为输出
#新建一个model用于层可视化
dense1_layer_model = Sequential()
dense1_layer_model.add(Conv2D(64,(6,6),activation='relu',input_shape=input_shape,
                   kernel_initializer=W_init,kernel_regularizer=l2(2e-4),name='Conv_1'))
...
class Siamese_Loader:
    ...
    def test_oneshot(self,model,N,k,s="test",time=0,verbose=0):
        ...
        for i in range(k):
            ...
            #show one shot test data
            if (time == 1) and (i == 1):
               plt.figure(1) 
               for b_i in range(2*N):
                   plt.subplot(8, 5, b_i+1)
                   if b_i < N:
                       plt.imshow(inputs[0][b_i,:,:,:]) #cmap='rgb'
                   else:
                       plt.imshow(inputs[1][b_i-N,:,:,:]) #cmap='rgb'
               plt.show()
...
for i in range(1, n_iter):
    ...
    ######################################################################
    #show train/validation batch
    #if i == 1:
    #     plt.figure(1) 
    #     for b_i in range(64):
    #         plt.subplot(8, 8, b_i+1)
    #         if b_i < 32:
    #             plt.imshow(test_inputs[0][b_i,:,:,:]) #cmap='rgb'
    #         else:
    #             plt.imshow(test_inputs[1][b_i-32,:,:,:]) #cmap='rgb'
    #     plt.show()
    #######################################################################          
    #one shot test
    if i % evaluate_every == 0:
        print("evaluating")
        val_acc = loader.test_oneshot(siamese_net,N_way,n_test,'test',i/evaluate_every,verbose=True)
        vacc.append(val_acc)
        if val_acc >= best:
            print("saving")
            ...
            #verfiy whether load weights 
            #print (dense1_layer_model.get_weights())
            #print (convnet.layer[0].get_weights())
            dense1_output = dense1_layer_model.predict(data)  
  
            print (dense1_output.shape)  
            print (dense1_output[0])  
            for p_i in range(64):
                pic = dense1_output[0][:,:,p_i]
                
                pic_path = "%s%s%s"%(layerout_path,str(p_i),'.jpg')
